...
// SchemaVersion is the read-envelope version (contract §10; append-only output contract).
...
// EmitEnvelope wraps read data in the stable `{schemaVersion, data, nextCursor?}` envelope
// (spec.md output contract). --select and --limit apply to the inner data, not the envelope.
// JSON gets the full envelope; plain/tsv get just the data rows (nextCursor noted on stderr)
// so the human path stays clean.
func (w *Writer) EmitEnvelope(data any, nextCursor string) error {
	return w.EmitEnvelopeWith(data, nextCursor, nil)
}

// EmitEnvelopeWith is EmitEnvelope plus extra top-level envelope fields (e.g. search's
// scope:public|self). Extra keys must not collide with schemaVersion/data/nextCursor.
func (w *Writer) EmitEnvelopeWith(data any, nextCursor string, extra map[string]any) error {
	...
	if nextCursor != "" {
		w.Info("nextCursor: %s", nextCursor)
	}
	for k, v := range extra {
		w.Info("%s: %v", k, v)
	}
